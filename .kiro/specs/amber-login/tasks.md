...
- [x] 8.2 エラーダイアログの実装
    ...
    - 汎用エラーダイアログ ✓
    - ユーザー拒否エラーダイアログ ✓
    - AlertDialogを使用したダイアログ実装（Amber未インストール、汎用エラー）
    - タイムアウトエラー検出と再試行ボタン表示ロジック
    - onDismissDialog、onInstallAmber、onRetryコールバック追加
    - 5つのダイアログ関連テスト実装済み
    - 3つのプレビュー関数追加（Amber未インストール、エラー、タイムアウト）
    - _Requirements: 1.2, 1.5, 5.1, 5.4_

- [x] 8.3 ログイン成功時のナビゲーション
    - ログイン成功メッセージの表示
    - メイン画面への画面遷移
    - _Requirements: 3.3_
...
- [x]* 9.3 MainScreenの単体テスト
    - pubkey表示テスト (Tests 2, 3, 10, 11) ✓
    - ログアウトボタン表示テスト (Tests 1, 4, 6, 12) ✓
    - ナビゲーションテスト (Tests 7, 8, 9) ✓
    - 12テスト実装済み (全要件カバー)
    - _Requirements: 2.3, 3.4, 3.5_
...
- [x]* 13.1 主要ユーザーフローのUIテスト
    - ログインフロー（ログイン画面 → ログインボタンタップ → ローディング表示 → メイン画面遷移） ✓
    - Amber未インストールエラーフロー ✓
    - ログアウトフロー（メイン画面 → ログアウトボタンタップ → ログイン画面遷移） ✓
    - UserFlowTest.kt作成 (6テスト実装済み)
    - テストカバレッジ:
        - Test 1: ログイン画面表示 (ボタン表示確認)
        - Test 2: ログインボタンタップ時のローディング表示
        - Test 3: ログイン成功後のメイン画面遷移
        - Test 4: Amber未インストール時のエラーダイアログ表示
        - Test 5: Amber未インストールダイアログの閉じる操作
        - Test 6: ログアウト時のログイン画面遷移
    - Note: Android Instrumentation Testのため、デバイス/エミュレーター上で実行
