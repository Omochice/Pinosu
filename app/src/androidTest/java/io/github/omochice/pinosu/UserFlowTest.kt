...
class UserFlowTest {
  ...
  fun `Nip55Signer not installed flow step1 should display error dialog`() {
    ...
    composeTestRule.onNodeWithText(getTestString(R.string.button_install)).assertIsDisplayed()

    composeTestRule.onNodeWithText(getTestString(R.string.button_close)).assertIsDisplayed()
  }
  ...
  fun `logout flow step1 should navigate to login screen on logout`() {
    ...
    composeTestRule
        .onNodeWithContentDescription(getTestString(R.string.cd_open_menu))
        .performClick()

    composeTestRule.waitUntil(timeoutMillis = 3000) {
      composeTestRule
          .onAllNodesWithText(getTestString(R.string.menu_logout))
          .fetchSemanticsNodes()
          .isNotEmpty()
    }
    ...
    composeTestRule.waitUntil(timeoutMillis = 3000) {
      composeTestRule
          .onAllNodesWithText(getTestString(R.string.button_login_with_nip55))
          .fetchSemanticsNodes()
          .isNotEmpty()
    }
    composeTestRule
  ...
  fun `app restart when not logged in should display login screen`() {
    ...
    composeTestRule.onNodeWithText(getTestString(R.string.title_bookmarks)).assertDoesNotExist()
  }
}
